...
export default function CloudflareDeploymentDashboard() {
 ...
 useEffect(() => {
        ...
        fetch(`/api/get-deploy-config?projectName=${projectName}`)
            .then((res) => res.json())
            .then((data) => {
                console.log("✅ Config loaded:", data);
                setProjectConfig(data);
  ...
  const triggerBuild = async (configParam) => {
        ...
        try {
            const res = await fetch("/api/build-and-serve", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
  ...
  const getBuildPhases = () => {
    const basePhases = [
      ...
      },
      {
        title: "Building application",
        icon:
          deploymentStatus === "building" && buildProgress > 50 ? (
            <Clock className="text-yellow-400 w-5 h-5 animate-spin" />
          ) : deploymentStatus === "success" ? (
            <CheckCircle className="text-green-500 w-5 h-5" />
      ...
      },
      {
        ...
        icon:
          deploymentStatus === "building" && buildProgress > 80 ? (
            <Clock className="text-yellow-400 w-5 h-5 animate-spin" />
          ) : deploymentStatus === "success" ? (
    ...
    if (deploymentStatus === "success") {
      basePhases.push({
        ...
        duration: "2s",
        status: "completed",
        logs: ["🔄 Invalidating edge cache...", "✅ Cache cleared across all regions"],
      })
    }
  ...
  return (
    <div className="bg-black text-white min-h-screen">
      ...
      <div className="md:ml-64">
        <div className="px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pt-16 md:pt-8">
          {/* Header with enhanced breadcrumb */}
          <div className="mb-6 sm:mb-8">
            <div className="text-sm text-gray-400 flex items-center space-x-2 mb-4 overflow-x-auto">
              <span>
              ...
              <span className="text-white font-medium whitespace-nowrap">Services</span>
            </div>
            <h3 className="text-2xl sm:text-3xl font-bold text-white">Deployment Dashboard</h3>
          </div>

          {/* Enhanced Deployment Info Card */}
          <div className="bg-[#18181b] rounded-xl border border-gray-700/50 p-4 sm:p-6 mb-6 shadow-2xl backdrop-blur-sm">
            ...
            <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start mb-6 space-y-4 lg:space-y-0">
              <div>
                <h2 className="text-xl sm:text-2xl font-bold mb-2">Deployment Details</h2>
              ...
              <button
                onClick={() => triggerBuild(projectConfig)}
                disabled={isBuilding || !projectConfig}
                className="bg-gradient-to-r from-blue-600 to-purple-600 hover:from-blue-700 hover:to-purple-700 disabled:from-gray-600 disabled:to-gray-700 px-4 sm:px-6 py-2 sm:py-3 rounded-lg text-sm font-medium flex items-center justify-center transition-all duration-300 shadow-lg hover:shadow-xl disabled:cursor-not-allowed transform hover:scale-105 disabled:scale-100 w-full lg:w-auto"
              >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 sm:gap-6 lg:gap-8">
              <div className="space-y-4">
                <div className="bg-[#27272a] rounded-lg p-4 border border-gray-700/50">
                  ...
                  <span className="text-white font-mono text-sm break-all">{projectConfig?.repo || "Loading..."}</span>
                </div>
                <div className="bg-[#27272a] rounded-lg p-4 border border-gray-700/50">
                  <div className="flex items-center justify-between mb-3">
                  ...
                  <span className="text-white font-mono text-sm">{projectConfig?.productionBranch || "main"}</span>
                </div>
                <div className="bg-[#27272a] rounded-lg p-4 border border-gray-700/50">
                  <div className="flex items-center justify-between mb-3">
                  ...
                  <a
                   href={`https://${projectConfig?.projectName}.vijaypvk.dev`}
                    target="_blank"
                    className="text-blue-400 hover:text-blue-300 underline transition-colors flex items-center text-sm break-all"
                    rel="noreferrer"
                  >
                    <Eye className="w-4 h-4 mr-1 flex-shrink-0" />
                    <span className="break-all">View Preview</span>
          ...
          {/* Enhanced Tabs */}
          <div className="flex space-x-1 mb-6 bg-gray-800/30 rounded-lg p-1 backdrop-blur-sm border border-gray-700/50 overflow-x-auto">
            {TABS.map((tab) => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-3 sm:px-4 py-2.5 text-sm font-medium rounded-md transition-all duration-200 flex-1 whitespace-nowrap min-w-0 ${
                  activeTab === tab
          ...
          {activeTab === "Build log" ? (
                      ...
                      >
                        ...
                        >
                          ...
                          <div className="flex items-center space-x-3 flex-shrink-0">
                            ...
                            <span className="text-gray-400 text-xs sm:text-sm font-mono">{phase.duration}</span>
                          </div>
                        </div>
                        {isExpanded && (
                            ...
                            <div className="p-4 font-mono text-xs sm:text-sm space-y-2">
                              {phase.logs.map((log, logIdx) => (
                                <div key={logIdx} className="flex items-start space-x-3">
                                  <span className="text-gray-500 text-xs mt-0.5 w-12 sm:w-16 flex-shrink-0">
            ...
            <div className="bg-gradient-to-br from-gray-800/50 to-gray-900/50 rounded-xl border border-gray-700/50 p-6 sm:p-8 text-center backdrop-blur-sm">
              ...
              <h3 className="text-lg sm:text-xl font-semibold mb-2">{activeTab}</h3>
              <p className="text-gray-400 mb-4 text-sm sm:text-base">
                No {activeTab.toLowerCase()} configured for this project.
              </p>
